...
const AllBlogBanner = () => {
  ...
  const API_URL = `${API_BASE_URL}/api/blog-banner`;
  ...
  useEffect(() => {
    const fetchBanner = async () => {
      try {
        ...
        toast.error("Failed to load blog banner");
      } finally {
        setLoading(false);
      }
    };
  ...
  const deleteBanner = async () => {
    ...
    try {
      ...
      setBanner(null);
      toast.success("Blog banner deleted successfully");
    } catch {
  ...
  return (
    <div className="container py-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <h4 className="fw-semibold mb-0">Blog Banner</h4>
      ...
      {!banner && (
        <div className="card text-center p-4 shadow-sm border-0">
          <p className="text-muted mb-0">No blog banner uploaded</p>
        </div>
      )}
