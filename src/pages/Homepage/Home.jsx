...
function Home() {
  ...
  /* ================= CONNECT WITH US ================= */
  useEffect(() => {
    const fetchConnectGallery = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/gallery/get-all-gallery`);
        const result = await res.json();

        if (res.ok && result.success && result.data.length > 0) {
          // CMS has only ONE gallery
          setConnectData(result.data[0]);
        }
      } catch (error) {
        console.error("Failed to load Connect With Us data");
      }
    };
  ...
  return (
    <>
              ...
              >
                {section.hasImage && (
                  <div
                    ...
                    style={{ backgroundImage: `url(${section.imagePath})` }}
                  >
              ...
              <div className="row">
                ...
                <div className="col-md-6">
                  <div className="LooksAbout">
                    <h1 className="AboutHead">{whyJoinUs.title}</h1>
        ...
        <div className="container-fluid">
          ...
          <div className="section-header">
            ...
            <p className="section-subtitle">
              Explore our comprehensive beauty and wellness courses designed to
      ...
      {/* ***************teacher********************** */}
      {connectData && (
        <section className="ConnectSection py-5">
          <div className="container">
            {/* TITLE */}
            <h1 className="Connect text-center mb-2">{connectData.title}</h1>

            {/* DESCRIPTION */}
            <p className="ConnectPara text-center mb-4">
              {connectData.description}
            </p>

            {/* IMAGES GRID */}
            <div className="row g-3">
              {connectData.images.slice(0, 9).map((img, index) => (
                <div key={index} className="col-6 col-md-4">
                  <div className="connect-img-box">
                    <img
                      src={`${API_BASE_URL}/${img.replace(/\\/g, "/")}`}
                      alt={`Connect gallery ${index + 1}`}
                      className="img-fluid"
                    />
                  </div>
                </div>
